...
function isTriangle(a, b, c) {
  ...
  const sortedTriangle = triangle.sort((x, y) => y - x);
  const ans = sortedTriangle[0] < sortedTriangle[1] + sortedTriangle[2];
  return ans;
}
...
function doRectanglesOverlap(rect1, rect2) {
  const x1 = rect1.left;
  const y1 = rect1.top;
  const x2 = x1 + rect1.width - 1;
  const y2 = y1 + rect1.height - 1;
  const x3 = rect2.left;
  const y3 = rect2.top;
  const x4 = x3 + rect2.width - 1;
  const y4 = y3 + rect2.height - 1;
  const xStart = Math.max(x1, x3);
  const xEnd = Math.min(x2, x4);
  const yStart = Math.max(y1, y3);
  const yEnd = Math.min(y2, y4);
...
function findFirstSingleChar(str) {
  for (let i = 0; i < str.length; i += 1) {
    if (str.indexOf(str[i]) !== -1 && str.indexOf(str[i]) === str.lastIndexOf(str[i])) {
      return str[i];
    }
  }
  return null;
...
function isCreditCardNumber(ccn) {
  ...
  const oddEven = ccnStr.length % 2; // odd = 0, even = 1
  let sum = 0;
...
function getDigitalRoot(num) {
  ...
  const numStr = String(num).split('').map((digit) => Number(digit));
  const sum = numStr.reduce((acc, digit) => acc + digit, 0);
...
function isBracketsBalanced(str) {
  ...
  let isBalanced = true;
  arr.forEach((br) => {
    if (Object.keys(BRACKETS).includes(br)) {
      stack.push(br);
    } else if (br === BRACKETS[stack[stack.length - 1]]) {
      stack.pop();
    } else {
      isBalanced = false;
      return false;
